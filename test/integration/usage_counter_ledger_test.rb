...
class UsageCounterLedgerTest < ActionDispatch::IntegrationTest
  ...
  test "Identity usage_history returns historical records across periods" do
    ...
    RSB::Entitlements::UsageCounter.create!(countable: identity, metric: "api_calls", period_key: "2026-02-11", plan: plan, current_value: 50, limit: 100)
    RSB::Entitlements::UsageCounter.create!(countable: identity, metric: "api_calls", period_key: "2026-02-12", plan: plan, current_value: 75, limit: 100)
  ...
  test "plan upgrade with continue mode carries over usage" do
    ...
    assert_equal 60, new_counter.current_value, "Usage should carry over from Basic plan (continue mode, same period type)"
  ...
  test "admin usage monitoring trend page loads" do
    ...
    identity = create_test_identity
    plan = create_test_plan(limits: { "api_calls" => { "limit" => 100, "period" => "daily" } })
    RSB::Entitlements::UsageCounter.create!(countable: identity, metric: "api_calls", period_key: "2026-02-12", plan: plan, current_value: 50, limit: 100)
    RSB::Entitlements::UsageCounter.create!(countable: identity, metric: "api_calls", period_key: "2026-02-13", plan: plan, current_value: 75, limit: 100)
