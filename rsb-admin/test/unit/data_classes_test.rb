...
class ColumnDefinitionTest < ActiveSupport::TestCase
  test "build with defaults" do
    ...
    assert_equal [:index, :show], col.visible_on
  end
  ...
  test "build with custom options" do
    col = RSB::Admin::ColumnDefinition.build(:status, label: "State", sortable: true, formatter: :badge, link: false, visible_on: [:index])
...
class FilterDefinitionTest < ActiveSupport::TestCase
  ...
  test "apply with proc scope" do
    custom_scope = ->(rel, val) { "filtered_#{val}" }
...
class FormFieldDefinitionTest < ActiveSupport::TestCase
  test "build with defaults" do
    ...
    assert_equal [:new, :edit], field.visible_on
  end

  test "build with custom options" do
    field = RSB::Admin::FormFieldDefinition.build(:email, type: :email, required: true, hint: "User email", visible_on: [:new])
...
class PageRegistrationTest < ActiveSupport::TestCase
  ...
  test "action_keys returns list of action key symbols" do
    ...
    assert_equal [:index, :by_metric], page.action_keys
  end
