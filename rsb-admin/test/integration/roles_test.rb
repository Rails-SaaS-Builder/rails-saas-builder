...
class RolesTest < ActionDispatch::IntegrationTest
  ...
  test "create saves a role with valid data" do
    ...
    assert_not_nil role
    assert_equal({ "articles" => ["index", "show"] }, role.permissions)
  ...
  test "destroy fails when role has admin users" do
    ...
    RSB::Admin::AdminUser.create!(email: "linked-#{SecureRandom.hex(4)}@example.com", password: "password123", password_confirmation: "password123", role: role_with_users)
